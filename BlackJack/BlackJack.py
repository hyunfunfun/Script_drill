...
class BlackJack:
    ...
    def deal(self):
        ...
        self.cardDeck = [i for i in range
        (52)]
    ...
    def hitDealer(self, n):
        ...
        # 파이썬은 라벨 이미지 레퍼런스를 갖고 있어야 이미지가 보임
        self.LcardsDealer[self.dealer.inHand() - 1].image = p
        self.LcardsDealer[self.dealer.inHand() - 1].place(x=250 + 30 * (n+1), y=150)
        PlaySound('sounds/cardFlip1.wav', SND_FILENAME)
    ...
    def pressedAgain(self):
        ...
        for card_label in self.LcardsPlayer:
            card_label.destroy()
        for card_label in self.LcardsDealer:
            card_label.destroy()
